...
export function LiveTimingBanner() {
  ...
  if (loading || timingData.length === 0) {
    return null
  }

  return (
    <div className="bg-[#15151E] border-b border-[#2A2A3A] py-2 sm:py-3 overflow-hidden sticky top-0 z-10">
      <div className="container mx-auto px-2 sm:px-4 lg:px-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            <div
              className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${
                isLive ? "bg-[#E10600] animate-pulse" : "bg-gray-500"
              }`}
            ></div>
            <Badge
              variant="outline"
              className="bg-[#2A2A3A] text-white border-[#3D3D4D] uppercase font-bold px-2 sm:px-3 text-xs sm:text-sm"
            >
              {isLive ? "LIVE" : "LATEST SESSION"}
            </Badge>
          </div>

          <div className="flex-1 overflow-x-auto w-full scrollbar-thin scrollbar-thumb-[#3D3D4D] scrollbar-track-[#15151E]">
            <div className="flex gap-3 sm:gap-4 md:gap-6 whitespace-nowrap min-w-max py-1">
              {timingData.slice(0, 10).map((driver) => (
                <div key={driver.driver} className="flex items-center gap-1 sm:gap-2 md:gap-3 group">
                  <div className="flex items-center gap-1 sm:gap-2">
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                      ...
                      <span className="font-medium text-xs sm:text-sm md:text-base group-hover:text-[#E10600] transition-colors">
                        {driver.driver}
                      </span>
                    </div>
                  </div>
                  <span
                    className={`text-xs sm:text-sm md:text-base font-mono ${getSectorClass(driver.lastLap)}`}
                  >
                    {driver.lastLap}
                  </span>
                  <span className="text-[0.65rem] sm:text-xs md:text-sm text-gray-400">{driver.gap}</span>
                </div>
              ))}
            </div>
          </div>
